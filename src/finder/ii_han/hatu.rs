...
mod valid {
    ...
    fn valid_hatu() {
        ...
        let hand = [
            Mentsu::Koutsu(
                Tile { tile_type: TileType::Dragon, number: 2 },
                random(),
            ),
        ...
        assert_eq!(Hatu::validate(&field, &winning_hand, &status), Some(("役牌:發".to_string(), 1)), "{:?}", hand);
    }
}
...
mod invalid {
    ...
    fn hatu_as_janto() {
        ...
        let winning_hand = from_hand(hand);
        let status = random_status();
        assert_eq!(Hatu::validate(&field, &winning_hand, &status), None, "{:?}", hand);
    }
}
