...
mod valid {
    ...
    fn valid_tanyao() {
        let hand = [
            ...
            generate_chuchan_pi_mentsu(),
            generate_chuchan_pi_mentsu(),
            Mentsu::Janto(Tile { tile_type: random_suhai_tile_type(), number: rand::thread_rng().gen_range(2..=8) }),
        ];
        let winning_hand = from_hand(hand);
        let field = random_field();
        let status = random_status();
        assert_eq!(Tanyao::validate(&field, &winning_hand, &status), Some(("断么九".to_string(), 1)), "{:?}", hand);
    }
}
...
mod invalid {
    ...
    fn has_jihai() {
        ...
        let field = random_field();
        let status = random_status();
        assert_eq!(Tanyao::validate(&field, &winning_hand, &status), None, "{:?}", hand);
    }
    ...
    fn has_chuchan() {
        let hand = [
            Mentsu::Koutsu(
                Tile { tile_type: random_suhai_tile_type(), number: 1 },
                random(),
            ),
            generate_chuchan_pi_mentsu(),
            generate_chuchan_pi_mentsu(),
