...
mod valid {
    ...
    fn valid_chinitu() {
        ...
        assert_eq!(Chinitu::validate(&random_field(), &from_hand(hand), &random_status()), Some(("清一色".to_string(), 6)), "{:?}", hand);
    }
    ...
    fn valid_chinitu_naki() {
        ...
        assert_eq!(Chinitu::validate(&random_field(), &from_hand(hand), &random_status()), Some(("清一色".to_string(), 5)), "{:?}", hand);
    }
}
...
mod invalid {
    ...
    fn valid_honitu() {
        ...
        assert_eq!(Chinitu::validate(&random_field(), &from_hand(hand), &random_status()), None, "{:?}", hand);
    }
}