...
mod count {
    ...
    fn one_in_shuntu() {
        ...
        field.dora = vec![Tile { tile_type: TileType::Manzu, number: 2 }];

        assert_eq!(Omote::validate(&field, &WINNING_HAND, &random_status()), Some(("ドラ".to_string(), 1)));
    }

    #[test]
    fn dora_anko() {
        let mut field = random_field();
        field.dora = vec![Tile { tile_type: TileType::Wind, number: 3 }];

        assert_eq!(Omote::validate(&field, &WINNING_HAND, &random_status()), Some(("ドラ".to_string(), 3)));
    }

    #[test]
    fn kantu_moro_nori() {
        let mut field = random_field();
        field.dora = vec![Tile { tile_type: TileType::Pinzu, number: 2 }];

        assert_eq!(Omote::validate(&field, &WINNING_HAND, &random_status()), Some(("ドラ".to_string(), 4)));
    }

    #[test]
    fn should_not_print_in_zero() {
        let mut field = random_field();
        field.dora = vec![Tile { tile_type: TileType::Wind, number: 4 }];

        assert_eq!(Omote::validate(&field, &WINNING_HAND, &random_status()), None);
    }
}
