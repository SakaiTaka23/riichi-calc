...
impl Sananko {
    fn is_ron_sananko(WinningHand { hand, winning_tile, .. }: &WinningHand) -> bool {
        let shuntu = hand
            .clone()
            .into_iter()
            .filter(|m| {
        ...
        let shuntu_mentsu = [
            shuntu.tile(),
            Tile { tile_type: shuntu.tile().tile_type, number: shuntu.tile().number + 1 },
            Tile { tile_type: shuntu.tile().tile_type, number: shuntu.tile().number + 2 },
        ];
...
mod valid {
    ...
    use crate::finder::test_utils::{random_field, random_janto, random_kantsu, random_koutsu, random_shuntu, random_status};
    ...
    fn existing_sananko() {
        ...
        let winning_hand = WinningHand { hand, winning_tile: hand[3].tile(), red_tile: 0 };
        let mut status = random_status();
        status.win_method = WinMethod::Ron;

        assert_eq!(Sananko::validate(&random_field(), &winning_hand, &status), Some(("三暗刻".to_string(), 2)), "{:?}", hand);
    }
    ...
    fn complicated_sananko() {
        let hand = [
            Mentsu::Koutsu(Tile { tile_type: TileType::Manzu, number: 3 }, false),
            Mentsu::Shuntsu(Tile { tile_type: TileType::Manzu, number: 2 }, false),
            Mentsu::Koutsu(Tile { tile_type: TileType::Souzu, number: 6 }, false),
            Mentsu::Koutsu(Tile { tile_type: TileType::Souzu, number: 7 }, false),
...
mod invalid {
    ...
    fn ron_sananko() {
        ...
        assert_eq!(Sananko::validate(&random_field(), &winning_hand, &status), None, "{:?}", hand);
    }
