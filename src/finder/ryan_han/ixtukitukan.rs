...
impl YakuBase for Ixtukitukan {
    fn validate(_: &Field, hand: &WinningHand, _: &Status) -> Option<(String, u8)> {
        ...
        if Self::check_shuntu(manzu) |
            Self::check_shuntu(pinzu) |
            Self::check_shuntu(sozu) {
...
mod valid {
    ...
    use crate::finder::test_utils::{from_hand, random_field, random_status, random_suhai_tile_type, random_tile};
    ...
    fn valid_ixtukitukan() {
        ...
        let hand = [
            ...
            Mentsu::Shuntsu(Tile { tile_type: color, number: 4 }, false),
            Mentsu::Shuntsu(Tile { tile_type: color, number: 7 }, false),
            Mentsu::Koutsu(random_tile(), false),
            Mentsu::Janto(random_tile()),
        ];
        assert_eq!(Ixtukitukan::validate(&random_field(), &from_hand(hand), &random_status()), Some(("一気通貫".to_string(), 2)), "{:?}", hand);
    }
    ...
    fn kuisagari_ixtukitukan() {
        let color = random_suhai_tile_type();
        let hand = [
            Mentsu::Shuntsu(Tile { tile_type: color, number: 1 }, true),
            Mentsu::Shuntsu(Tile { tile_type: color, number: 4 }, false),
            Mentsu::Shuntsu(Tile { tile_type: color, number: 7 }, false),
            Mentsu::Koutsu(random_tile(), false),
            Mentsu::Janto(random_tile()),
        ];
        assert_eq!(Ixtukitukan::validate(&random_field(), &from_hand(hand), &random_status()), Some(("一気通貫".to_string(), 1)), "{:?}", hand);
    }
}
...
mod invalid {
    ...
    fn different_color() {
        let hand = [
            Mentsu::Shuntsu(Tile { tile_type: TileType::Souzu, number: 1 }, true),
            Mentsu::Shuntsu(Tile { tile_type: TileType::Souzu, number: 4 }, false),
            Mentsu::Shuntsu(Tile { tile_type: TileType::Manzu, number: 7 }, false),
            Mentsu::Koutsu(random_tile(), false),
            Mentsu::Janto(random_tile()),
        ];
        assert_eq!(Ixtukitukan::validate(&random_field(), &from_hand(hand), &random_status()), None, "{:?}", hand);
    }
}
