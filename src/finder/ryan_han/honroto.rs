            ...
            match mentsu {
                ...
                Mentsu::Janto(tile) => {
                    if (tile.tile_type == TileType::Manzu || tile.tile_type == TileType::Pinzu || tile.tile_type == TileType::Souzu)
                        ...
                        return None;
                    } else if tile.tile_type == TileType::Dragon || tile.tile_type == TileType::Wind { yakuhai_count += 1; }
                }
            }
        }
...
mod valid {
    ...
    fn valid_honroto() {
        ...
        assert_eq!(Honroto::validate(&random_field(), &winning_hand, &random_status()), Some(("混老頭".to_string(), 2)), "{:?}", hand);
    }
}
...
mod invalid {
    ...
    fn is_chinroto() {
        ...
        let winning_hand = from_hand(hand);
        assert_eq!(Honroto::validate(&random_field(), &winning_hand, &random_status()), None, "{:?}", hand);
    }
}
